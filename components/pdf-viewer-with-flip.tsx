...
interface PDFViewerWithFlipProps {
  pdfUrl: string
  courseId: string
  onSessionUpdate?: (durationSeconds: number, currentPage: number) => void
}

export const PDFViewerWithFlip = ({ pdfUrl, courseId, onSessionUpdate }: PDFViewerWithFlipProps) => {
  ...
  useEffect(() => {
    ...
    // Simple page flip sound using data URI
      ...
      <div className="relative mx-auto w-full max-w-5xl">
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              <iframe
                ...
                onLoad={() => {
                  console.log("[v0] PDF loaded:", pdfUrl)
                  setPdfLoaded(true)
                }}
                onError={() => {
                  console.log("[v0] PDF failed to load:", pdfUrl)
                }}
                title="PDF Viewer"
              />
            </motion.div>
        ...
        <div className="absolute bottom-8 left-8 right-8 flex items-center justify-between">
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="h-14 w-14 rounded-full bg-[#16130F]/90 text-[#F3C77A] backdrop-blur-sm hover:bg-[#16130F] shadow-xl"
          >
      ...
      <div className="mx-auto w-full max-w-5xl">
        <div className="h-2 overflow-hidden rounded-full bg-[#2E261D]">
          <div
            ...
            style={{ width: `${(currentPage / Math.max(totalPages, currentPage)) * 100}%` }}
          />
        </div>
      </div>
    </div>
